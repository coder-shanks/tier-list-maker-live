...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-zinc-200 dark:border-zinc-800/80 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <SimpleTooltip content="Choose or switch category template" side="bottom">
            <button
              type="button"
              onClick={() => setTemplateOpen(true)}
              className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-zinc-100 dark:bg-zinc-900/90 hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-200 text-xs font-semibold border border-zinc-300 dark:border-zinc-800 transition-all shadow-2xs"
            >
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <div className="flex items-center bg-zinc-100 dark:bg-zinc-900/90 border border-zinc-300 dark:border-zinc-800 rounded-xl p-0.5 shadow-2xs">
            <SimpleTooltip content="Undo previous action" shortcut="⌘Z" side="bottom">
              <button
                ...
                onClick={undo}
                disabled={!canUndo()}
                className="p-1.5 rounded-lg text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-200 dark:hover:bg-zinc-800 disabled:opacity-25 disabled:hover:bg-transparent transition-colors"
              >
          ...
          >
            <button
              type="button"
              onClick={toggleTheme}
              className="p-2 rounded-xl bg-zinc-100 dark:bg-zinc-900/90 hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-300 dark:border-zinc-800 transition-colors shadow-2xs"
            >
              <HugeiconsIcon
          ...
          <SimpleTooltip
            content={
              previewMode
                ? 'Exit Preview Mode (Restore all editor controls)'
                : 'Preview Mode (Hide editor buttons for clean presentation)'
            }
            side="bottom"
          >
            <button
              ...
              className={`p-2 rounded-xl border text-xs font-semibold flex items-center gap-1.5 transition-all shadow-2xs ${
                previewMode
          ...
          <SimpleTooltip content="Export Tier List as PNG or JSON" shortcut="⌘E" side="bottom">
            <Button
              ...
              className="gap-1.5 font-bold bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white shadow-md shadow-indigo-500/25"
            >
