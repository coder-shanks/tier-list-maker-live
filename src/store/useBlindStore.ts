...
import type { BlindChallengeConfig, BlindChallengeHistoryEntry, BlindChallengeMode } from '../lib/types'
  ...
  persist(
    (set, get) => ({
      ...
      assignBlindCurrentItem: (targetTierId: string) => {
        const { isActive, currentItemId, mode, tierCaps, queue, lockedItemIds, history } = get()
