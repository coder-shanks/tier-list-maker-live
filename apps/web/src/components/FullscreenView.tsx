      ...
      <div className="pointer-events-auto flex flex-wrap items-center gap-2 p-1.5 sm:p-2 rounded-2xl bg-card/95 border border-border/80 text-foreground shadow-2xl backdrop-blur-xl ring-1 ring-border/50 transition-all">
        ...
        <Popover>
          ...
          <PopoverContent
            side="top"
            align="center"
            className="w-56 p-2.5 space-y-1.5 bg-card text-foreground border-border rounded-xl shadow-xl"
          >
              ...
              {themeOptions.map((th) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span
                      className={`w-3 h-3 rounded-full ${th.bgClass} border ${th.borderClass}`}
                    />
                    <span>{th.label}</span>
                  </div>
                  {presentationTheme === th.id && (
