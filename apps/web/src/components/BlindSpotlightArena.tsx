  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 inset-x-0 p-2 sm:p-2.5 text-center pointer-events-none">
        ...
        {item.subtitle && (
          <p className="text-[10px] text-zinc-300 truncate mt-0.5">{item.subtitle}</p>
        )}
      </div>
    </div>
  )
...
export default function BlindSpotlightArena() {
  ...
  const percentComplete =
    totalItems > 0 ? Math.round((placedCount / totalItems) * 100) : 0
            ...
            <div className="w-full space-y-2 pt-2">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 w-full max-w-xl mx-auto">
                {tiers.map((t) => {
                  ...
                  const isFull =
                    mode === 'hardcore' && cap !== undefined && currentCount >= cap
