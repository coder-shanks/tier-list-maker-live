  ...
  const handleCopyClipboard = async () => {
    ...
    try {
      ...
      if (blob && navigator.clipboard && window.ClipboardItem) {
        ...
        setErrorMessage(
          'Clipboard image copy not supported in this browser. Please download PNG instead.',
        )
      }
      setIsExporting(false)
    } catch (err) {
  ...
  const handleImportJson = (e: React.FormEvent) => {
    ...
    try {
      ...
      if (parsed.tiers && parsed.items && parsed.containers) {
        ...
        setErrorMessage(
          'Invalid tier list JSON structure: missing tiers, items, or containers.',
        )
      }
    } catch {
          ...
          <TabsContent value="image" className="space-y-3 pt-3">
            <div className="p-3.5 rounded-lg bg-secondary/60 border border-border text-center space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground max-w-xs mx-auto">
                Renders a crisp, watermark-ready 2x PNG of your board, perfect for sharing
                on Discord, Reddit, and Twitter/X.
              </p>
            </div>
