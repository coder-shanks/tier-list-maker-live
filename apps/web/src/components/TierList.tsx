...
export default function TierList() {
  ...
  const percentRanked =
    totalItemsCount > 0 ? Math.round((rankedCount / totalItemsCount) * 100) : 0
      ...
      {isAllRanked && !previewMode && (
        <div className="flex items-center justify-between p-3 px-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-xs font-semibold animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex items-center gap-2">
            ...
            <span>
              All <strong className="font-mono">{totalItemsCount}</strong> items placed!
              Tier list is complete.
            </span>
          </div>
          <Button
            size="xs"
      ...
      >
        ...
        <div className="p-4 sm:p-5 bg-card border-b border-border flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* Ranked Counter */}
            <SimpleTooltip
              content={`${rankedCount} of ${totalItemsCount} items ranked`}
              side="bottom"
            >
              <div
                tabIndex={0}
                role="status"
                className="px-2.5 py-1.5 rounded-lg bg-secondary border border-border/80 text-xs font-semibold flex items-center gap-2"
              >
                <span className="text-muted-foreground">
                ...
                <Badge
                  variant="secondary"
                  className="text-[10px] px-1.5 py-0 font-mono font-bold bg-foreground/10 text-foreground border-0"
                >
                  {percentRanked}%
                </Badge>
              </div>
            ...
            {/* Pool Counter */}
            <SimpleTooltip
              content={`${unassignedCount} unranked items remaining`}
              side="bottom"
            >
              <div
                tabIndex={0}
                role="status"
                className="px-2.5 py-1.5 rounded-lg bg-secondary border border-border/80 text-xs font-semibold flex items-center gap-1.5"
              >
      ...
      {!previewMode && (
        <div className="flex items-center justify-between px-1 pt-1">
          ...
          <p className="text-[11px] text-muted-foreground font-mono hidden sm:block">
            Tip: Drag items or click 3-dots to quick-assign • Click tier headers to
            customize
          </p>
        </div>
      )}
